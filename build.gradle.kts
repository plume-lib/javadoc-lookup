...
application { mainClass = "org.plumelib.javadoclookup.CreateJavadocIndex" }
...
tasks.withType<JavaCompile>().configureEach {
  ...
  // `jvmArgs` is nullable, so build a new list rather than adding to the existing one.
...
// Javadoc generates a CSS import of a font that is not distributed alongside the documentation, so
// remove the import.  Cannot use "ant.replaceregexp", because the Ant builder is reachable only
// through the "Project" object, which the configuration cache forbids a task action from using.
//
// The removal is written inline in the task action, rather than being factored out into a function
// or a top-level variable of this script, because both of those are members of the script object,
// and a task action that used one would capture that object, which the configuration cache forbids.
// A variable that is local to the configuration block below, by contrast, is captured by value.
tasks.withType<Javadoc>().configureEach {
  val javadocOptions = options as StandardJavadocDocletOptions
  javadocOptions.isNoTimestamp = true
  javadocOptions.quiet()
  val dejaVuFontImport = Regex("""@import url\('(?:resources/)?fonts/dejavu\.css'\);[ \t]*""")
  doLast {
    val javadocDir = (this as Javadoc).destinationDir
    ...
    javadocDir
      .walkTopDown()
      .filter { file ->
        file.isFile && (file.name.endsWith(".css") || file.name.endsWith(".html"))
      }
      .forEach { file ->
...
val strictJavadoc: (Javadoc) -> Unit = { javadocTask ->
  val javadocOptions = javadocTask.options as StandardJavadocDocletOptions
  javadocOptions.addBooleanOption("Xdoclint:all", true)
  javadocOptions.addBooleanOption("Xwerror", true)
}
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // The marker is a variable local to this configuration block, rather than a top-level variable
    // of this script, because a top-level variable is a member of the script object, and the task
    // action below would capture that object, which the configuration cache forbids.
